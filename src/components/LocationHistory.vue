...
<template>
  <Card
    class="w-full flex flex-col bg-gray-200 border border-white h-full max-h-[300px] md:max-h-[650px] overflow-y-auto">
    <Title :size="'sm'">Riwayat lokasi</Title>
    <div class="mt-4 w-full h-full">
      <div v-if="userInfo.loadingGetLocation">Loading</div>
      <div v-for="value in locationHistory" :key="value.location.latitide"
        class="p-2 rounded-lg bg-white mb-2 flex flex-col gap-1">
        <div class="flex justify-between items-center">
          <div class="font-bold">
            <Title :size="'md'">{{ value.location.city }}</Title>
          </div>
          ...
          <div class="flex flex-col text-right">
            ...
            <p class="text-xs text-gray-500">
              {{ new Date(value.weather[0][0].datetime).toLocaleDateString([], {
        ...
        <div class="flex flex-col gap-1">
          ...
          <!-- <span v-if="value.approximate" class="text-xs text-amber-700 bg-amber-50 rounded-md px-2 py-0.5 self-start">
